defmodule Dayron.ConfigTest do
  ...
  test "parses a valid config" do    
    {otp_app, adapter, logger, config} = Config.parse(Dayron.Repo, otp_app: :dayron_test, adapter: Dayron.ConfigTest.FakeAdapter)
  ...
  test "returns a default adapter if nothing is config" do
    {otp_app, adapter, _, _} = Config.parse(Dayron.Repo, otp_app: :dayron_test)
    assert otp_app == :dayron_test
    assert adapter == Dayron.HTTPoisonAdapter
  end

  test "returns default value for get_logger?" do
    {_, _, _, config} = Config.parse(Dayron.Repo, otp_app: :dayron_test)
  ...
  test "returns application logger config" do
    ...
    {_, _, logger, config} = Config.parse(Dayron.Repo, otp_app: :dayron_test)
